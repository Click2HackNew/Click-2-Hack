...
# ★★★★★ नया Feature 17: Keep-Alive Ping ★★★★★
@app.post("/api/device/keep_alive")
async def keep_alive_ping(data: dict):
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    if device_id:
        # Device का last_seen update करें
        cursor.execute(
            "UPDATE devices SET last_seen = ? WHERE device_id = ?",
            (current_time, device_id)
        )
        conn.commit()
        print(f"✅ Keep-alive ping received from {device_id} at {current_time}")
    
    conn.close()
    return {"status": "success", "message": "Keep-alive acknowledged"}
